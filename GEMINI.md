...
1.  **リリースの完全性 (Release Integrity)**:
    - AI の振る舞いやロジックに直接影響を与える「コアファイル（プロンプト、スクリプト、憲法等）」に変更を加えた際は、必ずバージョン番号を更新せよ。
    - プロジェクト固有の検証スクリプトやテストを用い、この整合性を物理的に証明せよ。
2.  **チェックリストの提示**: 
    - 検証結果に基づき、以下の 4 項目をユーザーに報告せよ。
    - [ ] **Cleanup**: 一時ファイル、一時ブランチ等の削除状況。
    - [ ] **Doc Sync**: 物理ファイル構成と `README.md` 等の同期状況。
    - [ ] **Versioning**: コアファイル変更に伴う `gemini-extension.json` の更新状況。
