...
- **パス解決の堅牢性 (Path Robustness)**:
    - サーバー起動やファイル生成を行うスクリプトは、Git Worktrees 等の「隔離環境」での実行を常に想定せよ。
    - 外部から渡されたパス（例：`--project-dir .`）や相対パスは、ディレクトリ移動（`cd`）を行う前に必ず絶対パスに変換し、実行中の整合性を物理的に保証せよ。
