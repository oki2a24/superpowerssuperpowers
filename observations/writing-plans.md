# 知見：writing-plans (Antigravity固有)

<!-- IMPROVED_ON: 2026-03-10 | REASON: GPAC v3 統合。大規模タスクの自律的分割と、agy-sub を活用した安全なハンドオフを実現するため。 -->
...
5.  **GPAC v3 へのハンドオフ手順 (Execution Handoff Refinement)**:
    *   オリジナルの「6. 実行の引き渡し」において「並行セッション (別のセッション)」が選択された場合、または上記 4 の基準を満たす場合、以下の手順を**厳格に実行**してください。
        ...
        3.  `agy-sub spawn` コマンドを生成し、ユーザーに提示（または実行許可を求める）。
