...
1.  **物理的構造 (Physical Structure)**:
    ...
    - インストール後のパス（`~/.antigravity/extensions/...`）を常に意識せよ。
...
# 知見：writing-skills (Antigravity固有)
...
## 概要
`writing-skills` および `anthropic-best-practices.md` を使用する際、Antigravity CLI (agy) 環境における適切な解釈とツールの読み替えを行うためのガイド。

## 外部出典ドキュメントの読み替え (Source Adaptation)
このプロジェクトのスキル作成ガイドラインは、Anthropic 社の Claude 向けベストプラクティスに基づいています。ドキュメント内の記述は、以下のマッピングに従って Antigravity CLI (agy) 環境へ適応させてください。

### 用語・概念の読み替え
| オリジナル (Claude 向け) | Antigravity CLI (agy) における解釈 | 備考 |
| :--- | :--- | :--- |
| **Claude** | **Antigravity / エージェント** | 自身への指示として受け取る |
| **Claude Haiku** | **Antigravity Flash** | 高速・経済的モデル |
| **Claude Sonnet** | **Antigravity Pro** | バランスモデル |
| **Claude Opus** | **Antigravity Ultra / Advanced** | 強力な推論モデル |
| **CLAUDE.md** | **ANTIGRAVITY.md** | プロジェクト憲法・規約ファイル |
| **~/.claude/skills/** | **~/.antigravity/skills/** | グローバルスキルディレクトリ |

### ツール名の読み替え
| Claude Code ツール | Antigravity CLI (agy) 対応ツール |
| :--- | :--- |
| `Read` | `read_file` |
| `Write` | `write_file` |
| `Edit` | `replace` |
...
### 補助ツールの役割
- **render-graphs.js**: 既存の `.dot` 形式から Mermaid への移行期間における、図解レンダリングの補助ツールとして一時的に同梱されています。最終的にはすべての図解を Mermaid 形式に統一し、外部スクリプトへの依存を排除することを目指します。

## AIエージェントへの指示 (Antigravity固有)
- **ソースの尊重**: `anthropic-best-practices.md` などの外部出典ドキュメントを編集する際は、安易に「Claude」を「Antigravity」に置換せず、上記の読み替えガイドを優先せよ。
...
- **セッション管理の区別**:
    ...
    - **スキルの認識（再起動）**: `agy --resume latest` を使い、履歴を保持したまま新しいスキルを読み込ませる行為。
- **実証テストの義務化**: スキルの検証において、単なるシミュレーションの報告は認められない。必ず具体的なファイル操作やタスク遂行を伴う「実証テスト」を実行し、その証拠を提示すること。
- **ローカル・アダプテーションの保護**: 移植済みスキルをアップデートする際は、既存の `## ローカル・アダプテーション (Antigravity固有)` セクション（本知見ファイル等）を必ず確認し、新バージョン適用後も一貫性を維持すること。
