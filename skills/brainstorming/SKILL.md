...
意図を伝えてユーザーが承認するまで実装を開始してはならないという承認ゲートは不変です。セレモニーの規模はタスクに応じてスケールしますが、承認ゲートは常に存在します。
</HARD-GATE>

## 三つのパス

最初の質問の前に、要求がどれだけのプロセスを必要とするかを分類し、声に出して分類を宣言してください — 例えば「これは限定された変更に見えるので、ここで短いデザインを提示し、仕様書は作成しない」など — 人間パートナーがそれを上書きできるようにします：

- **Spike** — 実現可能性の質問（「できるか？」「可能か？」「とりあえず動けば良い」）で、出力はコードではなく回答。2-3文で質問と探査計画を提示し、合意を得てから、正確性が許す限り安価に調査。設計ドキュメントなし、仕様書なし。知見は推奨として報告し、作ったものは破棄物と明示。
- **Bounded** — このリポジトリに既存のコードに対する限定された変更: 新しいフラグ、小さなエンドポイント、1ファイル修正。アプリの種類を理解しているだけでは不十分 — 変更するフローが読めるほど既に存在していること。既存フローが無い場合は限定されたものではない。重要な明確化質問をし、チャット内で短いデザイン（数文〜数段落）を提示し、停止。実装は人間パートナーがそのデザインに Yes と言った後だけ開始。仕様書なし、実装計画ドキュメントなし。承認ゲートはアーキテクチャの場合と同じくらい厳格。
- **Architectural** — 新プロジェクト、新サブシステム、コンポーネントの構成を再編成したり他が依存するインターフェースを変更する変更。完全なプロセスに従う: 質問、アプローチ、セクション化されたデザイン、記述された仕様、その後 writing-plans スキル。

2つのパスの間で迷ったら、重い方を取る。ラチェットは一方向: タスク中に隠れた複雑性が発見されたらパスをアップグレードし、停止して宣言し、ステップアップする。ミドルタスクでダウングレードすることはない。

## アンチパターン: 「承認が不要なほど単純だ」

すべてのパスは、人間パートナーが実装前に意図を承認することで終了します。ToDo リスト、単一機能のユーティリティ、設定変更 — デザインはチャットで2文でも良いが、必ず提示して承認を得なければなりません。

### レッドフラッグ

| 思考 | 現実 |
|------|------|
| "これは単純すぎて設計が不要" | 単純とは短い設計を意味し、設計なしではない。チャットで2文、その後承認。 |
| "Bounded と呼んで仕様をスキップする" | ラベルで作業をスキップしようとするのは疑念そのもの — 重いパスを取る。 |
| "Bounded でデザインは自明、読んでいる間に開始する" | ゲートは承認であり、設計の長さではない。提示してから承認を受けるまで停止。 |
| "この種のアプリは理解している、だから Bounded" | Bounded はリポジトリを測るもので、あなたの慣れではない。新しいプロジェクトは既存フローが無いため Architectural。 |
| "Spike が動作した、コードを残す" | Spike の出力は回答。残すのは新しい要求 — 分類し直す。 |
| "複雑になったが、ほぼ完了 — 再分類不要" | 隠れた複雑性はタスク中にパスをアップグレード。停止して宣言する。 |
| "Spike を承認した、だから続く変更も承認済み" | 各タスクは独自の分類と独自の承認が必要。 |
...
2. **ジャストインタイムでのビジュアル・コンパニオンの提案** — 事前ではなく、実際にビジュアルで見せたほうが分かりやすい具体的な質問が発生した時点で、他の質問と混ぜず単独のメッセージで提案します。承認されたらブラウザタブが自動的に開きます（下の「ビジュアル・コンパニオン」セクションを参照）。
...
- YAGNI（You Aren't Gonna Need It）を徹底し、すべてのアプローチやデザインから不要な機能や設計を削除してください。
...
**コンパニオンの提案（ジャストインタイム）:** 事前に提案しないでください。質問が単なるUIの「話題」ではなく、実際に言葉で説明するよりも見せたほうが明らかに分かりやすい（具体的なモックアップ、レイアウト、図解などの）場面になるまで待ってください。そのような場面に初めて遭遇した際、単独のメッセージで一度だけ提案します：
> 「この次の部分は視覚的にお見せしたほうが分かりやすいかもしれません。ブラウザのタブでモックアップ、図、比較資料などを作成できます。この機能は新しく、トークンを多く消費する可能性があります。よろしければブラウザタブを開きますが、いかがでしょうか？」

**この提案は、必ず単独のメッセージで行わなければなりません。** 明確化のための質問、要約、その他のコンテンツと絶対に混ぜないでください。メッセージには上記の提案のみを含め、ユーザーの返答を待ってください。ユーザーが承認した場合は、サーバーを `--open` フラグ付きで起動し、最初の画面で自動的にブラウザが開くようにします。拒否された場合は、テキストのみでブレインストーミングを継続し、ユーザーから要求されない限り再提案はしないでください。
