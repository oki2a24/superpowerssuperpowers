...
独立したコンテキストを持つ専門エージェントにタスクを委任することで、指示とコンテキストを正確に構成し、タスクの成功を確実にします。サブエージェントはあなたのセッションのコンテキストや履歴を継承すべきではなく、必要なものだけを構築して提供します。これにより、あなた自身のコンテキストを調整作業のために温存することもできます。
...
## Antigravity CLI (agy) での実現方法

Antigravity CLI (agy) 自体はシングルスレッドですが、以下の手法で「並行性」を実現できます。

1. **マルチ・ワークツリー戦略 (Human-driven Parallelism):**
   - `using-git-worktrees` を使い、問題ごとに独立したワークツリーを作成します。
   - 複数のターミナルを開き、それぞれのワークツリーで Antigravity CLI (agy) を起動して、独立した課題を同時に依頼します。
2. **バックグラウンド実行:**
...
## セッションからの具体的な実例 (2025-10-03 デバッグセッションより)
...
**ディスパッチ:**
これら3つのサブエージェントへのディスパッチを同じレスポンス内で同時に発行します（これらは並行して実行されます）：
...
※ 1つのレスポンス内で複数のディスパッチを呼び出すことで並行実行となります。レスポンスごとに1つずつ呼び出すと、順次（シーケンシャル）実行になります。
