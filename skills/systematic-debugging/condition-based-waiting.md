...
このディレクトリにある `condition-based-waiting-example.ts` を参照してください。実際のデバッグセッションで使用された、ドメイン固有のヘルパー関数（`waitForEvent`, `waitForEventCount`, `waitForEventMatch`）の完全な実装が含まれています。
...
- 3 つのファイルにまたがる 15 個の不安定なテスト（Flaky tests）を修正。
- テストのパス率：60% → 100% に向上。
- 実行時間：40% 高速化。
- 競合状態（Race Condition）の完全な解消。
