...
**個人のスキルは、プロジェクトの `skills/` ディレクトリ、またはグローバルディレクトリ（`~/.gemini/skills/`）に配置されます。**
...
**公式ガイダンス：** Anthropic公式のスキル作成ベストプラクティス（移植・適応版）については、[anthropic-best-practices.md](./anthropic-best-practices.md)を参照してください。このドキュメントを読み進める際は、**必ず [observations/writing-skills.md](../../observations/writing-skills.md) の読み替えガイドを併用し、Gemini CLI 環境向けに用語やツールを読み替えてください。**
