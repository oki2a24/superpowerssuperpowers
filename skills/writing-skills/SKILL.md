---
name: writing-skills
description: 新しいスキルを作成したり、既存のスキルを編集したり、デプロイ前にスキルの動作を検証したりする際に使用：効果的なスキルを作成・更新するためのガイド。
---
...
**個人のスキルは、プロジェクトの `skills/` ディレクトリ、またはグローバルディレクトリ（`~/.antigravity/skills/`）に配置されます。**
...
**公式ガイダンス：** Anthropic公式のスキル作成ベストプラクティス（移植・適応版）については、[anthropic-best-practices.md](./anthropic-best-practices.md)を参照してください。このドキュメントを読み進める際は、**必ず [observations/writing-skills.md](../../observations/writing-skills.md) の読み替えガイドを併用し、Antigravity CLI (agy) 環境向けに用語やツールを読み替えてください。**

## スキルとは何か？

**スキル**とは、実績のある技術、パターン、またはツールのためのリファレンスガイドです。将来の AI エージェントが、最も効果的なアプローチを自律的に見つけて適用するのを助けます。
...
- プロジェクト固有の規約（プロジェクトの`ANTIGRAVITY.md`に記述）
...
実戦で得られた知見を反映させる場合、`SKILL.md` を直接編集せず、`observations/` ディレクトリの適切な層（L1-L4）へ知見を蒸留（記録）します。これにより、元の移植内容（SSOT）とローカルでの知見を明確に分離し、移植元との同期を容易にします。
...
## 失敗のタイプに応じた形状の選択 (Match the Form to the Failure)

ガイダンスを執筆する前に、まずベースラインの失敗を分類してください。ある失敗タイプを防ぐのに有効な「形式」が、別の失敗タイプに対しては測定可能な形で裏目に出る（backfireする）ことがあります。

| ベースラインの失敗 | 適切な形式 (Right form) | 不適切な形式 (Wrong form) |
|---|---|---|
| プレッシャー下でルールをスキップ/違反する（分かっているのにやってしまう） | 禁止事項 ＋ 言い訳テーブル ＋ レッドフラグ（下記の「防弾化」を参照） | 緩いガイダンス（「〜を好む」、「〜を検討する」など） |
| ルールに従うが、出力形式が崩れる（長すぎるプロンプト、埋もれた判定、要件の繰り返し） | 肯定的なレシピまたは契約：出力が何であるか、その構成要素を順に明記する | 禁止事項のリスト（「〜を繰り返すな」、「〜を語るな」） |
| すでに出力しているものの中に、必要な要素が欠落している | 構造的：テンプレートに埋めるべき「必須 (REQUIRED)」のフィールドやスロットを明記する | テンプレート付近での散文（文章）によるリマインダー |
| 状況（条件）によって行動を変えるべきである | 観察可能な条件（述語）に紐づいた条件分岐（「もしbriefが存在するなら、それを参照する」） | 無条件のルール ＋ 例外条項 |

**形式の乱れの問題に対して、なぜ「禁止事項」が裏目に出るのか：** 「プロンプトを自己完結させる」といった対立するインセンティブがある場合、エージェントは「〜するな（don't X）」という禁止事項と妥協しよう（交渉しよう）とします。ディスパッチプロンプトのガイダンスに関する頭部テスト（head-to-head wording tests）では、禁止事項を用いたアームは、肯定的なレシピを用いたアームよりも明らかに望ましくない出力を多く生成し、ガイダンスなしのコントロール群よりも悪化する傾向がありました。したがって、自らマイクロテストを行って検証するまでは、決して安易に禁止事項を選択しないでください。レシピを設定すれば、交渉の余地はなくなります：出力が指定された形状に一致しているか、そうでないかだけです。

**選択した形式を執筆する際のルール：**
- **ニュアンスのある（曖昧な）例外条項を排除する。** 「重要でない限り〜するな」は交渉を再開させてしまいます。勝利したレシピに単一のニュアンス条項を追加しただけで、テスト結果が一貫したものからノイズだらぜのものへと劣化しました。実際の例外は、観察可能な条件を用いた条件分岐として表現してください。
- **免責条項は適用範囲を制限しない。** 「この制限はコードブロックには適用されない」と書いても、エージェントは依然としてコードブロックを抑制しようとします。出力の一部を免責する必要がある場合は、ルールがそこまで届かないように構造を再設計してください。

## 正当化に対するスキルの防弾化

規律を強制するスキル（TDDなど）は、正当化に抵抗する必要があります。エージェントは賢く、圧力下で抜け穴を見つけます。

**心理学的な補足：** 説得技術がなぜ機能するのかを理解することは、それらを体系的に適用するのに役立ちます。権威、コミットメント、希少性、社会的証明、結束の原則に関する研究の基礎については、[persuasion-principles.md](./persuasion-principles.md)を参照してください。
...
フルプレッシャーシナリオの実行は最終ゲートですが、イテレーションごとの実行が遅く、コストがかかります。まずはマイクロテストを使用して、文言そのものを検証してください。

1. **1回の呼び出しにつき、クリーンなコンテキストを1つ使用する** — 生のAPI呼び出し、またはAPIアクセスがない場合は単発 of サブエージェントを使用します。システムプロンプト ＝ ガイダンスが実際に組み込まれる現実的なコンテキスト（ガイダンス単体ではなく、スキル全体またはプロンプトテンプレート全体）を設定し、ユーザーメッセージ ＝ 違反を誘発するようなタスクを設定します。
2. **常に「ガイダンスなし」のコントロール（制御群）を含める** — コントロール群で失敗が発生しない場合は、修正すべき問題がありません。ガイダンスの執筆を中止してください。
3. **バリアントごとに5回以上実行する** — 単一のサンプルは嘘をつきます。
4. **フラグが立った一致項目をすべて手動で確認する** — プログラムによるスコアリングも可能ですが、テンプレートの残響や引用された反例が合格として誤検知されることがあります。自動カウントだけでは、失敗と成功の両方を過大評価してしまいます。
5. **ばらつき（Variance）を指標にする** — ガイダンスが適切に機能している場合、複数回実行した結果は同じ形状に収束します。5回の実行で5つの異なる解釈が生まれる場合、その文言はエージェントを拘束できていません。文言を追加する前に、形式を厳格に締め直してください。

マイクロテストは文言を検証するためのものであり、規律スキルにおけるプレッシャーシナリオの代わりにはなりません。

**テスト方法：** スキルの詳細なテスト方法については、[testing-skills-with-subagents.md](testing-skills-with-subagents.md) を参照してください。
...
**進捗管理: 以下の各チェックリスト項目のチェックボックス（`- [ ]`）を更新して進捗を追跡します。**
...
- [ ] ガイダンスの形式が失敗のタイプに一致している（「失敗のタイプに応じた形状の選択」を参照）
- [ ] 行動形成のためのガイダンスについて、ガイダンスなしのコントロール群に対して文言のマイクロテストを完了している（5回以上の実行、かつフラグの一致を手動で確認） — 純粋な参照スキルの場合は該当なし
...
1. **問題に遭遇する**（「テストが不安定だ」）
2. **スキルを検索する**（description を grep したり、カテゴリをブラウズする）
