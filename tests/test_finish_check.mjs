...
test('finish_check.mjs - 異常系: バージョン更新漏れを検知して非ゼロで終了すること', (t) => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(repoPath, 'agy-extension.json'), JSON.stringify({ version: "1.0.0" }));
    ...
    assert.match(result.stderr, /agy-extension\.json のバージョンが更新されていません/, 'バージョンエラーメッセージが出力されること');
  } finally {
    fs.rmSync(repoPath, { recursive: true, force: true });
  }
});
...
test('finish_check.mjs - 正常系: すべての条件を満たす場合に 0 で終了すること', (t) => {
  ...
  try {
    ...
    fs.mkdirSync(skillDir);
    fs.writeFileSync(path.join(skillDir, 'test-skill.md'), 'content');
    fs.writeFileSync(path.join(repoPath, 'agy-extension.json'), JSON.stringify({ version: "1.1.0" }));
...
test('finish_check.mjs - 異常系: ANTIGRAVITY.md の変更を検知して非ゼロで終了すること', (t) => {
  const repoPath = createTempRepo();
  try {
    fs.writeFileSync(path.join(repoPath, 'ANTIGRAVITY.md'), 'updated constitution');
    fs.writeFileSync(path.join(repoPath, 'agy-extension.json'), JSON.stringify({ version: "1.0.0" }));
    spawnSync('git', ['add', 'ANTIGRAVITY.md'], { cwd: repoPath });
    ...
    assert.notStrictEqual(result.status, 0, 'ANTIGRAVITY.md の変更がある場合は非ゼロで終了すべき');
  } finally {
    fs.rmSync(repoPath, { recursive: true, force: true });
  }
});
